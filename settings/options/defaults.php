...
if(!class_exists('Redux_Options') ){
    ...
    $fslashed_abs = trailingslashit(str_replace('\\','/', ABSPATH));
     ...
     function reduxCleanFilePath( $path ) {
    ...
    if(!defined('Redux_OPTIONS_DIR')) {
        define('Redux_OPTIONS_DIR', $redux_dir);
    }
    
    if(!defined('Redux_OPTIONS_URL')) {
        define('Redux_OPTIONS_URL', $redux_url);
    }
    ...
    class Redux_Options {
        ...
        */
        function __construct($sections = array(), $args = array(), $extra_tabs = array()) {    
			...
			if(floatval(get_bloginfo('version')) >= "3.8") {
				...
				$theme_menu_icon =  $this->url . 'img/theme-nectar.png';
			}
            ...
            $defaults['last_tab'] = '0';
            $defaults['menu_icon'] = $theme_menu_icon;
            $defaults['menu_title'] = __('Options', NECTAR_THEME_NAME);
            $defaults['page_icon'] = 'icon-themes';
            $defaults['page_title'] = __('Options', NECTAR_THEME_NAME);
            ...
            $defaults['help_sidebar'] = __('', NECTAR_THEME_NAME);
        ...
        */
        function _enqueue() {
			...
			wp_register_script( 
                'ace-editor-js', 
                $this->url . 'js/ace_editor/ace.js',
                array( 'jquery' ),
                time(),
                true
            );  

            wp_localize_script('redux-opts-js', 'redux_opts', array('reset_confirm' => __('Are you sure? You will lose all custom values.', NECTAR_THEME_NAME), 'opt_name' => $this->args['opt_name']));
        ...
        */
        function _options_page_html() {
            ...
            global $options;
            if(!empty($options['external-dynamic-css']) && $options['external-dynamic-css'] == 1) generate_options_css();
        }
